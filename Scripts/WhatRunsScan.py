...
def detect_web_technologies(ip, port, protocol="http", timeout=10, verbose=True):
    """
    ...
    if WEBTECH_AVAILABLE:
        ...
        # Fallback to basic header detection if webtech is not available
        try:
...
def scan_ports_and_services(ip, verbose=True):
    ...
    try:
        ...
        # For ports that might be running web services, detect technologies
        web_ports = []
        ...
        if web_ports:
            ...
            for port, protocol in web_ports:
                technologies = detect_web_technologies(ip, port, protocol, verbose=verbose)
...
def print_ip_summary(result):
    ...
    if port_count > 0:
        ...
        for port, info in result['ports'].items():
            ...
            print(f"Port {port}/{info['protocol']}: {info['service']}{status}")
    ...
    if format == 'json':
        ...
        with open(output_file, 'w') as f:
            ...
            for result in results_list:
                ...
                f.write("PORT      PROTOCOL  STATE   SERVICE                  FINDINGS\n")
                f.write("-" * 80 + "\n")
                ...
                for port, info in result['ports'].items():
                    ...
                    f.write(f"{port:<10}{info['protocol']:<10}{info['state']:<8}{info['service']:<25}{findings_text}\n")
